...
export const WORK: WorkCase[] = [
  {
    ...
    body: `<p>Every Fourth of July, Milander Park becomes one of Florida's biggest single-night gatherings — live music, cultural pride, and a fireworks finale that draws crowds from across the county. In 2021, the night was headlined by Cuban reggaeton stars <b>Gente de Zona</b>, and the whole show stood on Reflections Productions steel.</p><p>Reflections delivered the full production: an APEX GEN3 hydraulic stage in super beast mode, intelligent lighting rigs, and concert audio engineered so every corner of the park heard a stadium-grade mix. From load-in to strike, coordination between the artists, city officials and vendors ran through one production office — ours.</p><p>It's the kind of civic event where there is no second take: the anthem, the headliner and the fireworks happen once, on time, in front of everyone. That's the job Reflections has been doing for fifty years.</p>`,
    deployList: [
